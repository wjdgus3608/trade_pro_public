...
public class ShortTimeExitSellBot extends NormalBot implements ExitSellBot{
    ...
    public int sellFilter(Object coin, double nowPrice) {
        ...
        if(GlobalBotControll.getCoinMap().get(cName)!=SHORT_TIME_TYPE) return 0;
    ...
    public void sell() {
        ...
        for(Object obj : coinList) {
            ...
            if(sellFilter(coin,nowPrice)>=lostSellLimit){
                LogUtil.printLog("ShortTimeExitSellBot sell "+coin.getCName()+" "+coin.getBuyAvgPrice()+"->"+nowPrice);
                LogUtil.saveLog("ShortTimeExitSellBot sell "+coin.getCName()+" "+coin.getBuyAvgPrice()+"->"+nowPrice);
